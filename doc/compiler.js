/*Auto generated by Kirby v1.0.0 - Tue Jan 30 2018 01:27:31 GMT-0800 (PST)
...
const sf_DASH_dissoc_BANG = function(ast, env) {
  ...
  let ret = node_QUOT(ast);
  ret.add(
  [stripStd(KBSTDLR+"."+tx_STAR(symbol("dissoc!"), env)),
    "(", tx_STAR(kirbystdlibref.getProp(ast, 1), env), ",", tx_STAR(ast[2], env), ")"]);
  return ret;
};
...
const sf_DASH_assoc_BANG = function(ast, env) {
  ...
  for (let i = 2, sz = kirbystdlibref.count(ast), ____break = false; ((!____break) && (i < sz)); i = (i + 2)) {
    ...
    ret.add(
    [
    stripStd(KBSTDLR+"."+tx_STAR(symbol("assoc!"), env)),
      "(", obj, ",", tx_STAR(xfi(ast, ast[i]), env), ",", tx_STAR(xfi(ast, ast[i + 1]), env), ")"]);
  }
  return wrap(ret, "(", ")");
};
