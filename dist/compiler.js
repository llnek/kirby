/*Auto generated by Kirby v1.0.0 - Sat Jan 06 2018 18:08:35 GMT-0800 (PST)
...
const sf_DASH_deftype = function(ast, env) {
  ...
  for (let GS__23 = 0, GS__22 = false, ____break = false; ((!____break) && ((!GS__22) && (GS__23 < GS__21.length))); GS__23 = (GS__23 + 1)) {
    ...
    if ( ((!GS__22) && true) ) {
      (mtd = kirbystdlibref.symbol("method"), m1 = kirbystdlibref.getProp(m, 0), cn = [
        czname0,
        ".",
        m1
      ].join(""), pos = (((typeof (kirbystdlibref.getProp(m, 1)) === "string")) ?
        3 :
        2));
      [
        mtd,
...
const sf_DASH_require = function(ast, env) {
  ...
  for (let i = 1, sz = kirbystdlibref.count(ast), ____break = false; ((!____break) && (i < sz)); i = (i + 1)) {
    ...
    for (let i = 0, t = null, r = null, rs = null, f = null, m = null, sz = kirbystdlibref.count(refers), ____break = false; ((!____break) && (i < sz)); i = (i + 1)) {
      (r = refers[i], rs = [
        r
      ].join(""), v = tx_STAR(r, env), f = kirbystdlibref.getProp(rlib, parser.jsid(rs)), m = kirbystdlibref.getProp(mcs, rs));
      if (f) {
        t = typeid(f);
      }
      if (m) {
    ...
    for (let i = 0, t = null, r = null, rs = null, ro = null, f = null, m = null, sz = kirbystdlibref.count(renames), ____break = false; ((!____break) && (i < sz)); i = (i + 2)) {
      (ro = renames[i], e = tx_STAR(ro, env), r = renames[i + 1], rs = [
        r
      ].join(""), v = tx_STAR(r, env), f = kirbystdlibref.getProp(rlib, parser.jsid([
        ro
      ].join(""))), m = kirbystdlibref.getProp(mcs, [
        ro
      ].join("")));
      if (f) {
        t = typeid(f);
...
const sf_DASH_macro = function(ast, env) {
  ...
  (mname = kirbystdlibref.getProp(ast, 1), ast = [
