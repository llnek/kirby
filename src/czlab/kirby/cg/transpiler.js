/*Auto generated by Kirby - v1.0.0 czlab.kirby.cg.transpiler Wed Nov 22 2017 18:59:06 GMT+1100 (AEDT)*/
...
const sf_DASH_var = function(ast, env) {
  ...
  for (let i = 0, sz = kirbystdlibref.count(ast), ____break = false; ((!____break) && (i < sz)); i = (i + 2)) {
    (lhs = ast[i], rhs = ast[i + 1], rval = tx_STAR(rhs, env));
    if (symbol_QUERY(lhs)) {
      (lhs = txAtom(lhs), tmp = [cmd, " ", lhs, "= ", rval, ";\n"]);
      conj_BANG(keys, lhs);
      ret.add(tmp);
    } else {
      out = tnode();
  ...
  for (let pos = 0, sz = kirbystdlibref.count(args), ____break = false; ((!____break) && (pos < sz)); pos = (pos + 1)) {
    ...
    if (symbol_QUERY(e)) {
      if ( (ev === "&") ) {
        e = args[pos + 1];
        if (symbol_QUERY(e)) {
          ret.add(["let ", txAtom(e), "=slice(arguments,", pos, ");\n"]);
        } else {
          if (map_QUERY(e)) {
            destructMap();
          } else {
...
const transpileCode = function(codeStr, fname, options) {
  ...
  let G____13 = options;
  let source_DASH_map = G____13["source-map"];
  let format = G____13["format"];
  let cstr = "";
  let fopts = {};
    ...
    if (symbol_QUERY(e)) {
      if ( (e == "_") ) {
        ...
        if ( (e == "&") ) {
          ...
          if (true) {
            out = tnode();
            out.add(["let ", txAtom(e), "=", src, "[", [i].join(""), "];\n"]);
            ret.add(out);
          }
        }
      }
    ...
    if (keyword_QUERY(e)) {
      if ( ((e == "keys") || (e == "strs")) ) {
        ...
        for (let j = 0, sz = count(arr), ____break = false; ((!____break) && (j < sz)); j = (j + 1)) {
          e = arr[j];
          ret.add(["let ", txAtom(e), "=", src, "[\"", [e].join(""), "\"];\n"]);
        }
      } else {
        if (true) {
