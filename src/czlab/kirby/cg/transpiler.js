/*Auto generated by Kirby - v1.0.0 czlab.kirby.cg.transpiler Sun Nov 12 2017 02:10:05 GMT-0800 (PST)*/
...
function exprHint(ast, flag) {
  ...
  extendAttr(x, "____expr")["____expr"] = flag;
  return x;
}
  ...
  if ( (cmd == "with-meta") ) {
    ...
    if (cmd.startsWith(".-")) {
      ...
      if ( ("." === cmd.charAt(0)) ) {
        ...
        if (contains_QUERY(SPEC_DASH_OPS, cmd)) {
          ...
          if (rdr.REGEX.int.test(cmd)) {
            ...
            (ast = [symbol(cmd.charAt(0)), ast[1], parseInt(cmd.slice(1))], cmd = [ast[0]].join(""), ret = SPEC_DASH_OPS[cmd](ast, env));
          } else {
            if (true) {
              if (pair_QUERY(ast)) {
                txExpr(ast, env);
...
function writeDoc(doc) {
  return (doc ?
    (function() {
      (doc = doc.replace(rdr.REGEX.dquoteHat, ""), doc = doc.replace(rdr.REGEX.dquoteEnd, ""));
      return doc.split("\\n").map(function() {
        let ____args = Array.prototype.slice.call(arguments);
        return ["//", ____args[0], "\n"].join("");
      });
...
function sf_DASH_new(ast, env) {
  let ret = nodeTag(tnode(), ast);
  ret.add(txList(ast.slice(1), env));
  ret.prepend("new ");
  return ret;
}
