/*Auto generated by Kirby - v1.0.0 czlab.kirby.cg.transpiler Wed Nov 08 2017 19:39:41 GMT-0800 (PST)*/

var std = require("../bl/stdlib");
var hashmap = std["hashmap"];
...
let MODULE_DASH_VERSION,
  EXTERNS,
  SPEC_DASH_OPS,
  MATH_OP_REGEX;
MODULE_DASH_VERSION = "1.0.0";
EXTERNS = null;
SPEC_DASH_OPS = {};
MATH_OP_REGEX = /^[-+][0-9]+$/;
...
function simpleton_QUERY(ast) {
  return ((typeof (ast) === "string") || (typeof (ast) === "number") || (typeof (ast) === "boolean") || (ast === null));
}
          ...
          if (rdr.REGEX.int.test(cmd)) {
            {
              ...
              (ast = [std.symbol(cmd.charAt(0)), ast[1], parseInt(cmd.slice(1))], cmd = ast[0].toString(), ret = SPEC_DASH_OPS[cmd](ast, env));
            }
          } else {
            if (true) {
              {
...
function sf_DASH_lambda(ast, env) {
  ...
  let x,
    body;
  x = [kirbystdlibref.symbol("fn"), [], [kirbystdlibref.symbol("var"), kirbystdlibref.symbol("____args"), [kirbystdlibref.symbol("slice"), kirbystdlibref.symbol("arguments")]]];
  body = ast[1];
  if ( (0 === body.length) ) {
    body = [null];
  }
...
function sf_DASH_apply(ast, env) {
  ...
  ret.prepend("[");
  ret.add("]");
  ret.prepend([transpile_STAR(f, env), ".apply(this,"]);
  ret.add(")");
  return ret;
}
...
function sf_DASH_do(ast, env) {
  ...
  ret = nodeTag(tnode(), ast);
  stmtQ = stmt_QUERY(ast);
  body = exprHint(ast.slice(1), (!stmtQ));
...
function sf_DASH_range(ast, env) {
  ...
  transpileAtoms(ast, env);
  (len = ast.length, end = parseInt(ast[1]));
  if ( (len > 2) ) {
    {
      (start = parseInt(ast[1]), end = parseInt(ast[2]));
    }
  }
  if ( (len > 3) ) {
    step = parseInt(ast[3]);
...
function sf_DASH_var(ast, env, cmd) {
  ...
  for (let i = 0, ____break = false; ((!____break) && (i < ast.length)); i = (i + 2)) {
    if (symbol_QUERY(ast[i])) {
      conj_BANG(keys, ast[i]);
    }
  }
  ret.add(";\n");
  if ( (!(0 === kirbystdlibref.count(keys))) ) {
...
function handleFuncArgs(fargs, env) {
  ...
  fargs.forEach(function(arg) {
    return (symbol_QUERY(arg) ?
      conj_BANG(keys, arg) :
      ((symbol_QUERY(arg[0]) && (!arg[0].toString().startsWith("&")) && Array.isArray(arg[2])) ?
        (function() {
...
function sf_DASH_if(ast, env) {
  ...
  ret = nodeTag(tnode(), ast);
  stmtQ = stmt_QUERY(ast);
  a1 = exprHint(ast[1], (!stmtQ));
  a2 = exprHint(ast[2], (!stmtQ));
  a3 = ((ast.length > 3) ?
    ast[3] :
    null);
  elze = (a3 ?
...
function sf_DASH_str(ast, env) {
  ...
  if ( (args.length > 1) ) {
    ...
    if ( (!(0 === kirbystdlibref.count(args))) ) {
...
function sf_DASH_doseq(ast, env) {
      ...
      if ( (ev === "when") ) {
        {
          ...
          G__6 = ret;
          G__6.add(["if (!", escvar, ") {\n"]);
          G__6.add(["if (! ", transpile_STAR(args[(i + 1)], env), ") {continue;}\n"]);
          G__6.add("}\n");
          G__6;
        }
      } else {
        if ( (ev === "let") ) {
          ret.add(sf_DASH_var_DASH_let([symbol("var")].concat(args[(i + 1)]), env));
        } else {
          null;
        }
      }
  ...
  ret.add("}\n");
  ret.add("}\n");
  ret.prepend(["for(let ", escvar, "=false,", idxvar, "=0; ", "(!", escvar, " && ", idxvar, " < ", exprvar, ".length); ++", idxvar, ") {\n"]);
