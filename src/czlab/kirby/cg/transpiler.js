/*Auto generated by Kirby - v1.0.0 czlab.kirby.cg.transpiler Sat Nov 11 2017 17:25:34 GMT-0800 (PST)*/
...
function transpileList(ast, env) {
  ...
  if ( (cmd == "with-meta") ) {
    ...
    if (cmd.startsWith(".-")) {
      ret.add([transpile_STAR(ast[1], env), ".", transpileSingle(symbol(cmd.slice(2)))]);
    } else {
      if ( ("." === cmd.charAt(0)) ) {
...
function sf_DASH_var(ast, env, cmd) {
  ...
  for (let i = 0, rc = null, lhs = null, rhs = null, ____break = false; ((!____break) && (i < ast.length)); i = (i + 2)) {
    ...
    if (symbol_QUERY(lhs)) {
      (lhs = transpileSingle(lhs), tmp = ["= ", transpile_STAR(rhs, env), ";\n"]);
...
function sf_DASH_if(ast, env) {
  ...
  let elze = (a3 ?
    exprHint(a3, (!stmtQ)) :
    null);
  (a1 = transpile_STAR(a1, env), a2 = transpile_STAR(a2, env), elze = transpile_STAR(elze, env));
  if (stmtQ) {
    ret.add(["if (", a1, ") {\n", a2, ";\n}"]);
    if (a3) {
      ret.add([" else { \n", elze, ";\n}"]);
...
function transpileCode(codeStr, fname, options) {
  ...
  let source_DASH_map,
    format;
  source_DASH_map = G____18["source-map"];
  format = G____18["format"];
  let cstr = "";
  let fopts = {};
