/*Auto generated by Kirby - v1.0.0 czlab.kirby.bl.lexer Sat Oct 28 2017 18:20:59 GMT-0700 (PDT)*/

var tn= require("./tnode");
var std= require("./stdlib");
;
let REGEX;
REGEX= { noret: /^def\b|^var\b|^set!\b|^throw\b/,id: /^[a-zA-Z_$][\/.?\-*!0-9a-zA-Z_'<>#@$]*$/,id2: /^[*\-][\/.?\-*!0-9a-zA-Z_'<>#@$]+$/,float: /^[-+]?[0-9]+\.[0-9]+$/,int: /^[-+]?[0-9]+$/,hex: /^[-+]?0x/,dquoteHat: /^"/,dquoteEnd: /"$/,func: /^function\b/,slash: /\//g,query: /\?/g,bang: /!/g,plus: /\+/g,dash: /-/g,quote: /'/g,hash: /#/g,at: /@/g,less: /</g,greater: />/g,star: /\*/g,wspace: /\s/ };
;
//
...
return (testid_QUERY(name) ?
  [pfx,name].join("").replace(REGEX.query,"_QUERY").replace(REGEX.slash,".").replace(REGEX.bang,"_BANG").replace(REGEX.dash,"_DASH").replace(REGEX.quote,"_QTE").replace(REGEX.hash,"_HASH").replace(REGEX.plus,"_PLUS").replace(REGEX.at,"_AT").replace(REGEX.less,"_LT").replace(REGEX.greater,"_GT").replace(REGEX.star,"_STAR") :
  ((pfx === "") ?
    name :
    [pfx,name].join("")));
}
