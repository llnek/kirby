/*Auto generated by Kirby - v1.0.0 czlab.kirby.cg.transpiler - Wed Nov 22 2017 21:31:30 GMT+1100 (AEDT)*/
...
//
const node_QUOTE = function(obj, src) {
...
const destruct0 = function(cmd, lhs, rhs, env) {
  ...
  {
    let G____11 = keys;
    Object.entries(G____11).forEach(function(e) {
      return (function(v, k) {
...
const txTree = function(root, env) {
  let ret = node_QUOTE(tnode(), root);
  {
    let G____12 = root;
    for (let G____14 = 0, G____13 = false, ____break = false; ((!____break) && ((!G____13) && (G____14 < G____12.length))); G____14 = (G____14 + 1)) {
      let r = G____12[G____14];
      let tmp = tx_STAR(r, env);
      if ( (!true) ) {
        G____13 = true;
      }
      if ( ((!G____13) && some_QUERY(tmp)) ) {
        {
          ret.add([
            tmp,
            "\n"
...
const sf_DASH_deftype = function(ast, env) {
  ...
  for (let i = 0, pos = 0, sz = kirbystdlibref.count(mtds), ____break = false; ((!____break) && (i < sz)); i = (i + 1)) {
    ...
    pos = ((typeof (m[1]) === "string") ?
      3 :
      2);
    if ( ((m[0] == "constructor") && not_DASH_empty(args)) ) {
      {
        txExpr(args, env);
...
const sf_DASH_vardefs = function(ast, env) {
  ...
  if ( ((!private_QUERY) && (1 === rt.globalEnv().countNSPCache())) ) {
    {
      let G____16 = kks;
      Object.entries(G____16).forEach(function(e) {
        return (function(v, k) {
          return EXTERNS[k] = k;
        })(e[1], e[0]);
      });
...
const handleFuncArgs = function(fargs, env) {
  ...
  fargs.forEach(function(arg) {
    return (symbol_QUERY(arg) ?
      ...
      ((symbol_QUERY(arg[0]) && (![
        arg[0]
      ].join("").startsWith("&")) && Array.isArray(arg[2])) ?
        (function() {
  ...
  if (c) {
    {
      ...
      ret.add([
        ...
        txDo(exprHint(c.slice(2), (!stmtQ)), env),
        ";\n}\n"
      ]);
    }
  }
...
const sf_DASH_jscode = function(ast, env) {
  return node_QUOTE(tnodeEx([
    ast[1]
  ].join("").replace(rdr.REGEX.dquoteHat, "").replace(rdr.REGEX.dquoteEnd, "")), ast);
};
...
const cleanCode = function(code) {
  ...
  let arr = [];
  {
    let G____18 = code.split("\n");
    for (let G____20 = 0, G____19 = false, ____break = false; ((!____break) && ((!G____19) && (G____20 < G____18.length))); G____20 = (G____20 + 1)) {
      let s = G____18[G____20];
      null;
      if ( (!true) ) {
        G____19 = true;
      }
...
const transpile = function(code, file) {
  let G____23 = Array.prototype.slice.call(arguments, 2);
  let options = G____23[0];
  return transpileXXX(code, file, opt_QUERY__QUERY(options, {}));
};
//
