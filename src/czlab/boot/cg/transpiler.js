...
function destructStar(coll, out) {
  let rhs= types.symbol(gensym());
  let e;
  for (let i=0; i < coll.length; ++i) {
    e=coll[i];
    if (types.keyword_p(e) && e=="as") {
      rhs=coll[i+1];
      break;
    }
  }
  if (types.map_p(coll)) {
    out.add(destruct_Map(rhs, coll));
  } else if (types.vector_p(coll)) {
    out.add(destruct_Vec(rhs,coll));
  }
